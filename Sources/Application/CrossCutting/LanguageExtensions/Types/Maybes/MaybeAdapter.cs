...
public static class MaybeAdapter
{
    ...
    {
        if (maybe is None<T>) return None.Value;
    ...
    public static T Reduce<T>(
        ...
        Func<T> whenNone)
    {
        if (maybe is None<T>) return whenNone();
    ...
    {
        if (maybe is None<T>) return await whenNone();
    ...
    {
        if (maybe is None<T>) throw new ArgumentException("Maybe is none");
